...
function LunaUF:CreateConfig()
	...
	local function set(info, value)
		...
		if info.arg then
			db = db[info.arg]
			local name = string.match(info[#info],".*_(.*)")
			if value ~= db[name] then
				db[name] = value
				local unit = info[#info-1]
				if LunaUF.Units.headerUnits[unit] then
					LunaUF.Units:ReloadHeader(unit)
				else
					LunaUF.Layout:Reload(unit)
				end
			end
		else
	...
	local moduleOptions = {
		["healthBar"] = {
			...
			health_background = {
				...
				desc = L["Enable or disable the background."],
				type = "toggle",
				order = 3.2,
				arg = "healthBar",
			},
			health_backgroundAlpha = {
