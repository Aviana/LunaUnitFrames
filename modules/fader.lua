...
function Fader:CastStart(frame, event, unit, id)
	if( unit ~= "player" or activeCastID == id ) then return end --To be removed in classic due to only player casting events
	activeCastID = id
...
function Fader:Update(frame, event)
	...
	if( InCombatLockdown() or event == "PLAYER_REGEN_DISABLED" ) then
		startFading(frame, "in", LunaUF.db.profile.units[frame.unitType].fader.combatAlpha, LunaUF.db.profile.units[frame.unitType].fader.speedyFade)
	-- Player is casting, fade in
	elseif( frame.fader.playerCasting ) then
		startFading(frame, "in", LunaUF.db.profile.units[frame.unitType].fader.combatAlpha, true)
	...
	-- Nothing else? Fade out!
		...
		startFading(frame, "out", LunaUF.db.profile.units[frame.unitType].fader.inactiveAlpha, LunaUF.db.profile.units[frame.unitType].fader.speedyFade)
	end
end
